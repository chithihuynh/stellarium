...
\begin{figure}[tbp]
\centering\includegraphics[width=0.68\textwidth,trim=0 50 0 0,clip]{search_dialog_option_updated.png}
...
of objects whose name contains \emph{m}: \emph{M}iranda (UV), Psa\emph{m}athe (NX), U\emph{m}briel (UII), \ldots
% Not how it is currently showing:
% \emph{m}: Hau\emph{m}ea, \emph{M}iranda, U\emph{m}briel, \ldots 
...
At this time, your search result will be record. This will enable the search 
results to prioritize your most recent searches (which will also be italized 
and/or highlighted). Continuing with our example, open the Object Search window
again and re-enter \emph{m} to generate a list similar to: \emph{M}imas, \emph{M}aia, 
\emph{M}ars, \ldots. Since Mimas, was your most recent searched 
object, it will be the first name highlighted and italized.    

% Explain recent search history size
Shown in Fig. ~\ref{fig:gui:search:options}, you can change the size of your search history 
(ranging from 0-99) by moving to the Options tab of this panel.
To update the size, click \keys{OK}. To revert back to the previous size, click \keys{Cancel}. 
A recent search history size with a red coloring indicates that you changed the size without clicking either \keys{OK} or \keys{Cancel}. This means that the size has not updated yet. 
