...
\begin{figure}[tbp]
\centering\includegraphics[width=0.68\textwidth]{search_dialog_option_updated.png} % TODO_CH: remove "_update" 
...
%% TODO_CH: Note - the tab key does not go backward (even if use shift  + tab).  But the right and left arrow moves the highlighted position forward and backward respectively.

% Draft:
[......If you press the \key{\arrowkey{>}} or \key{\arrowkey{<}} keys, the selection 
will change to the next or previous item in the list, respectively. ...]
...
\begin{center}
	\emph{M}iranda (UV), Psa\emph{m}athe (NX), U\emph{m}briel (UII), \ldots
\end{center}
...
with \emph{m}:
\begin{center}
	\emph{M}aia, \emph{M}ars, \ldots 
\end{center}

The first item in this list, Maia, is
...
% TODO_CH: or the \key{\arrowkey{>}} to highlight 'Mimas' @ tabs

% Example with "recent searches" saved
% TODO_CH: right version number?    
After being directed to your searched object, 
that object (Mimas) will be record. \newFeature{v0.20.3}  
Therefore, the next time you search for an object, the most recent searches are prioritized.
For instance, continuing with our example, re-open the Search Window and re-enter \emph{m}. 
A similar list should generate, with Mimas appearing first:
\begin{center}
	\emph{M}imas, \emph{M}aia, \emph{M}ars, \ldots
\end{center}

% Explain recent search history size
% TODO_CH: right version number?    
Shown in Fig.~\ref{fig:gui:search:options}, the recent search history size can be changed 
in the Options tab of this panel. \newFeature{v0.20.3} 
The size can range from 0-99 objects.
To update the size, toggle to the number of choice. 
Update the size by clicking \keys{OK}. 
Else, click \keys{Cancel} to revert back to currently set size. 
A modified size number will appear red until \key{OK} or \key{Cancel} has been clicked. 
Indiciating, that the size has not been updated to the number currently shown.
...
Thi or angular ses advanced functionality, some of which is still in an experimental phase. 
